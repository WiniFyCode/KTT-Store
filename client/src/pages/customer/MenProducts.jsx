// MenProducts.jsx - Trang sản phẩm Nam
...
const MenProducts = () => {
  ...
  useEffect(() => {
    ...
    // Lọc chỉ lấy sản phẩm Nam
    const menProducts = {
      ...data,
      products: data.products.filter(product => product.gender === 'Nam')
    };
    setProductsData(menProducts);
  ...
  useEffect(() => {
    ...
    setCurrentPage(1);
  }, [productsData, selectedCategory, selectedTag, selectedPriceRange, searchTerm, sortBy]);
  ...
  return (
    <div className={`min-h-screen ${theme === 'tet' ? 'bg-red-50' : 'bg-gray-50'}`}>
      <PageBanner
        ...
        subtitle="Phong cách lịch lãm, đẳng cấp cho phái mạnh"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
              {currentProducts.map((product) => (
                <Link 
                  ...
                  className={`group relative overflow-hidden rounded-lg shadow hover:shadow-lg transition-shadow duration-300 ${
